  ...
  return (
    <Panel title="Hero video (below the navbar)" testId="admin-hero-video">
      <div className="grid gap-4">
        <div>
          <Label htmlFor="hv-url">YouTube URL</Label>
          <Input
            id="hv-url"
            value={url}
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Paste the URL only — embed/iframe HTML is rejected. The video id is extracted on the server.
            Watch, embed, shorts, live and youtu.be links all work.
          </p>
          {data?.video_id && (
            <p className="mt-1 text-xs font-medium text-brand-deep" data-testid="admin-hero-video-id">
              Current video id: {data.video_id}
            </p>
          )}
        </div>
        <div>
