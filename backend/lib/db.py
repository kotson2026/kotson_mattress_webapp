...
mongo_url = os.environ.get("MONGO_URL", "mongomock://localhost:27017")
db_name = os.environ.get("DB_NAME", "kotson")
...
if mongo_url.startswith("mongomock://"):
    ...
    try:
        ...
        test_client = MongoClient(mongo_url, serverSelectionTimeoutMS=1500)
        test_client.server_info()
        client = AsyncIOMotorClient(mongo_url)
        db = client[db_name]
    except Exception as exc:
        logger.warning("Could not connect to MongoDB at %s: %s. Using in-memory mongomock_motor.", mongo_url, exc)
        from mongomock_motor import AsyncMongoMockClient
        client = AsyncMongoMockClient()
        db = client[db_name]
